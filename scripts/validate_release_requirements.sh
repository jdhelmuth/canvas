...
INFO_PLIST="$ROOT/Canvas/Info.plist"
[ -f "$INFO_PLIST" ] || fail "Info.plist not found: $INFO_PLIST"
for key in NSPhotoLibraryUsageDescription NSPhotoLibraryAddUsageDescription NSLocationWhenInUseUsageDescription; do
  /usr/libexec/PlistBuddy -c "Print :$key" "$INFO_PLIST" >/dev/null || fail "Info.plist is missing $key"
done
ENTITLEMENTS="$ROOT/Canvas/Canvas.entitlements"
[ -f "$ENTITLEMENTS" ] || fail "entitlements file not found: $ENTITLEMENTS"
WEATHERKIT=$(/usr/libexec/PlistBuddy -c 'Print :com.apple.developer.weatherkit' "$ENTITLEMENTS" 2>/dev/null || true)
[ "$WEATHERKIT" = "true" ] || fail "WeatherKit entitlement is missing"

SETTINGS=$(mktemp)
trap 'rm -f "$SETTINGS"' EXIT HUP INT TERM
xcodebuild -project "$PROJECT" -target "$SCHEME" -configuration Release -showBuildSettings >"$SETTINGS"
...
if grep -Eq 'IconOptions|01-gallery-portal|02-memory-mosaic|03-quiet-window|04-woven-moments|05-memory-orbit|01-gallery-beam|02-folded-light|03-keepsake-stack|04-color-echo|05-canvas-lightwell|GENERATION_NOTES' "$PBXPROJ"; then
